#!/usr/bin/env python3
"""
Extract Frame Transformations for TPGMM - Cyclical Version

This script analyzes the training data to extract translation vectors between
the 3 coordinate frames (FR1, FR2, FR3) used in the cyclical TPGMM training.
...
def load_model_data(model_path):
    """Load the trained cyclical TPGMM model data."""
...
def analyze_frame_differences(all_trajectories, feature_names):
    ...
    """
    print("Analyzing cyclical frame differences...")
    ...
    for frame_idx, frame_name in enumerate(['FR1', 'FR2', 'FR3']):
        ...
        print(f"{frame_name} trajectory shape: {trajectories.shape}")
        ...
        if frame_idx == 0:  # Only print once
            first_positions = position_data[:, 0, :]  # First point of each cycle
            last_positions = position_data[:, -1, :]  # Last point of each cycle
            cyclical_diff = np.mean(np.linalg.norm(first_positions - last_positions, axis=1))
            print(f"Cyclical continuity check - mean position difference: {cyclical_diff:.8f}")
    ...
    return {
        ...
        'cyclical_modification': True  # Flag to indicate this is from cyclical data
    }
...
def plot_frame_analysis(analysis_results, save_dir="plots"):
    ...
    ax1.set_title('Right Ankle Position Distribution by Frame (Cyclical)')
    ...
    ax2.set_title('Left Ankle Position Distribution by Frame (Cyclical)')
    ...
    ax3.set_title('Translation Vectors - Right Ankle (Cyclical)')
    ...
    ax4.set_title('Translation Vectors - Left Ankle (Cyclical)')
    ax4.set_ylabel('Translation (m)')
    ax4.legend()
    ax4.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(f'{save_dir}/frame_transformation_analysis_cyclical.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"Cyclical frame analysis plot saved to {save_dir}/frame_transformation_analysis_cyclical.png")

def main():
    """Main analysis pipeline."""
    print("=== Frame Transformation Extraction - Cyclical Version ===")
    
    # Load cyclical model data
    pkls_dir = "/home/jemajuinta/ws/Gait-analysis-coupled/pkls"
    model_path = os.path.join(pkls_dir, "gait_tpgmm_model_cyclical.pkl")
    
    print(f"Loading cyclical model from: {model_path}")
    model_data = load_model_data(model_path)
    
    # Check if this is indeed a cyclical model
    if 'cyclical_modification' in model_data:
        print(f"Cyclical modification confirmed: {model_data['cyclical_modification']}")
    else:
    ...
    print(f"Cyclical analysis results saved to: {analysis_save_path}")
    
    # Print summary
    print("\n=== Translation Summary (Cyclical) ===")
    translations = analysis_results['translations']
    pos_features = analysis_results['position_features']
