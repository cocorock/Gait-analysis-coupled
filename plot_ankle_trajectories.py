...
def plot_ankle_positions(ankle_data):
    ...
    ax1.set_title('Left Ankle Trajectory (X vs Y Position)', fontsize=14, fontweight='bold')
    for i, frame_data in enumerate(ankle_data['positions']['left']):
        ...
        ax1.plot(frame_data['x'][0], frame_data['y'][0], 'o', 
                color=colors[i], markersize=8, label=f'{frame_data["frame"]} start')
        ax1.plot(frame_data['x'][-1], frame_data['y'][-1], 's', 
                color=colors[i], markersize=8, label=f'{frame_data["frame"]} end')
    ...
    ax2.set_title('Right Ankle Trajectory (X vs Y Position)', fontsize=14, fontweight='bold')
...
def plot_ankle_velocities(ankle_data):
    ...
    ax1.set_title('Left Ankle Velocity (X vs Y Velocity)', fontsize=14, fontweight='bold')
    ...
    ax1.set_xlabel('X Velocity (m/s)', fontweight='bold')
    ax1.set_ylabel('Y Velocity (m/s)', fontweight='bold')
    ax1.grid(True, alpha=0.3)
    ax1.legend()
    ax1.set_aspect('equal', adjustable='box')
    
    # Plot right ankle velocities
    ax2.set_title('Right Ankle Velocity (X vs Y Velocity)', fontsize=14, fontweight='bold')
    ...
    ax2.set_xlabel('X Velocity (m/s)', fontweight='bold')
    ax2.set_ylabel('Y Velocity (m/s)', fontweight='bold')
    ax2.grid(True, alpha=0.3)
    ax2.legend()
    ax2.set_aspect('equal', adjustable='box')
...
def main():
    ...
    plot_ankle_positions(ankle_data)
    
    print("Creating ankle velocity plots...")
    plot_ankle_velocities(ankle_data)
