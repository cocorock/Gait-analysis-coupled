...
def visualize_multiple_adaptations(time_baseline, mu_baseline, results_list, 
    ...
    # Overall title
    fig.suptitle(f'TPGMM Adaptability Test: {test_name} - FR3 (7 Variations)', 
...
def visualize_phase_space_plots(time_baseline, mu_baseline, results_list,
    ...
    for i, result in enumerate(results_list):
        ...
        # Close the trajectory by appending the first point
        vx_closed = np.append(mu_adapted[:, 2], mu_adapted[0, 2])
        vy_closed = np.append(mu_adapted[:, 3], mu_adapted[0, 3])
        ax.plot(vx_closed, vy_closed,
               linestyle=linestyle, linewidth=linewidth,
               color=colors[i], alpha=alpha, zorder=5)
    
    # Close baseline trajectory
    vx_baseline_closed = np.append(mu_baseline[:, 2], mu_baseline[0, 2])
    vy_baseline_closed = np.append(mu_baseline[:, 3], mu_baseline[0, 3])
    ax.plot(vx_baseline_closed, vy_baseline_closed, 'k--',
           linewidth=2.5, label='Baseline', alpha=0.7, zorder=10)
    ...
    for i, result in enumerate(results_list):
        ...
        # Close the trajectory by appending the first point
        vx_closed = np.append(mu_adapted[:, 7], mu_adapted[0, 7])
        vy_closed = np.append(mu_adapted[:, 8], mu_adapted[0, 8])
        ax.plot(vx_closed, vy_closed,
               linestyle=linestyle, linewidth=linewidth,
               color=colors[i], alpha=alpha, zorder=5)
    
    # Close baseline trajectory
    vx_baseline_closed = np.append(mu_baseline[:, 7], mu_baseline[0, 7])
    vy_baseline_closed = np.append(mu_baseline[:, 8], mu_baseline[0, 8])
    ax.plot(vx_baseline_closed, vy_baseline_closed, 'k--',
           linewidth=2.5, label='Baseline', alpha=0.7, zorder=10)
    ...
    # Overall title
    fig.suptitle(f'Phase Space Analysis: {test_name} - FR3 (7 Variations)', 
                fontsize=18, fontweight='bold', y=0.98)
