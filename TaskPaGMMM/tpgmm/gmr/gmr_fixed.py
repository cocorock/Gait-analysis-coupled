...
class GaussianMixtureRegression(RegressionModel):
    ...
    def _equation_5(self, translation: ndarray, rotation_matrix: ndarray):
        """
        Implements Calinon Equation 5: Transform Gaussians from local frames to global frame
        ...
        Where:
            - j: frame index
            - i: component index
            - A_t,j: rotation matrix for frame j
            - b_t,j: translation vector for frame j
            - mu(j)_i: mean of component i in frame j
            - Sigma(j)_i: covariance of component i in frame j

        This function transforms all Gaussians from their local frame representations
        ...
        print(f"eq5 Sorted means shape: {sorted_means.shape}")  # (num_frames, num_components, num_features)

        sorted_covariances = self._sort_by_input(
            self.tpgmm_covariances_,
            axes=[-2, -1],
        )
        print(f"eq5 Sorted covariances shape: {sorted_covariances.shape}")  # (num_frames, num_components, num_features, num_features)
        ...
        for frame_idx in range(num_frames):
            ...
            for comp_idx in range(num_components):
                # Mean transformation: xi_hat = A @ mu + b
                mu = sorted_means[frame_idx, comp_idx]  # (num_features,)
                xi_hat_[frame_idx, comp_idx] = A @ mu + b

                # Covariance transformation: Sigma_hat = A @ Sigma @ A^T
    ...
    def _equation_6(self, xi_hat_: ndarray, sigma_hat_: ndarray):
        """
        Implements Calinon Equation 6: Product of Gaussians (fusion across frames)

        Calinon Eq. 6:
            Sigma_hat_t,i = (Sum_{j=1}^P Sigma_hat(j)_t,i^(-1))^(-1)
            xi_hat_t,i = Sigma_hat_t,i * Sum_{j=1}^P (Sigma_hat(j)_t,i^(-1) * xi_hat(j)_t,i)
        ...
        This function combines the Gaussians from all frames (after transformation to
        global coordinates in equation 5) into a single Gaussian Mixture Model.
        """
        ...
        for comp_idx in range(num_components):
            ...
            for frame_idx in range(num_frames):
                ...
                # Accumulate precision matrices: Sum_j Sigma_hat(j)^(-1)
                precision_sum += precision

                # Accumulate weighted means: Sum_j (Sigma_hat(j)^(-1) @ xi_hat(j))
                weighted_mean_sum += precision @ xi

            # Fused covariance: inverse of sum of precisions
            # Sigma_hat = (Sum_j Sigma_hat(j)^(-1))^(-1)
            sigma_hat[comp_idx] = np.linalg.inv(precision_sum)

            # Fused mean: fused_covariance @ sum of weighted means
            # xi_hat = Sigma_hat @ (Sum_j Sigma_hat(j)^(-1) @ xi_hat(j))
