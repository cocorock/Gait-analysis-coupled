...
def extract_trajectories(data, subsample_factor=2):
    ...
    fr2_data = data['kinematics_data']['FR1']  # Note: You might want FR2 here
    ...
    for demo_fr1, demo_fr2 in zip(fr1_data['right_leg_kinematics'], 
        ...
        # Right leg data (from right_leg_kinematics or from main structure)
        ...
        # Left leg data (you mentioned left_leg_kinematics is empty, 
        # but we'll try to extract it if it exists, otherwise use placeholders)
        if 'left_ankle_pos' in demo_fr1 and len(demo_fr1['left_ankle_pos']) > 0:
            left_ankle_pos = np.array(demo_fr1['left_ankle_pos'])
        else:
            # Create placeholder with same shape as right ankle
            left_ankle_pos = np.zeros_like(right_ankle_pos)
            
        if 'left_ankle_vel' in demo_fr1 and len(demo_fr1['left_ankle_vel']) > 0:
            left_ankle_vel = np.array(demo_fr1['left_ankle_vel'])
        else:
            left_ankle_vel = np.zeros_like(right_ankle_vel)
        ...
        if 'left_ankle_pos' in demo_fr2 and len(demo_fr2['left_ankle_pos']) > 0:
            left_ankle_pos = np.array(demo_fr2['left_ankle_pos'])
        else:
            left_ankle_pos = np.zeros_like(right_ankle_pos)
            
        if 'left_ankle_vel' in demo_fr2 and len(demo_fr2['left_ankle_vel']) > 0:
