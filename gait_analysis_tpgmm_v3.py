...
def extract_trajectories(data, subsample_factor=2):
    ...
    subsample_factor : int
        Use every Nth point to speed up computation (default: 2)
        Higher values = faster but less data
    """
    print("\n" + "="*70)
    ...
    for demo_fr1, demo_fr2 in zip(fr1_data['right_leg_kinematics'], 
        ...
        traj_fr1 = np.vstack([
            right_ankle_pos[0, indices],           # x position
            right_ankle_pos[1, indices],           # y position
            right_ankle_vel[0, indices],           # x velocity
            right_ankle_vel[1, indices],           # y velocity
            np.deg2rad(ankle_right_deg[indices]),  # right ankle angle (rad)
            np.deg2rad(ankle_left_deg[indices]),   # left ankle angle (rad)
            time_vector                             # normalized time
        ]).T  # Shape: (n_subsampled, 7)
        
        trajectories_fr1.append(traj_fr1)
        
        # FR2 trajectory (same structure with subsampling)
    ...
    # Convert to arrays
    trajectories_fr1 = np.array(trajectories_fr1)  # (n_demos, n_subsampled, 7)
...
def main(data_path, subsample_factor=10):
    """
    ...
    -----------
    data_path : str
        Path to gait data JSON file
    subsample_factor : int
        Subsampling factor for trajectories (default: 10)
        Higher values = faster execution but less data
    """
    print("\n" + "="*70)
    ...
    trajectories_fr1, trajectories_fr2 = extract_trajectories(data, subsample_factor=subsample_factor)
    ...
    plt.tight_layout()
    plt.savefig('original_trajectories_2d.png', dpi=150, bbox_inches='tight')
    ...
    print(f"✓ Subsampling: {subsample_factor}x (faster execution)")
    print("✓ All visualizations saved")
    print("="*70)
    print("\nSpeed optimization:")
    print(f"  - Original data points per demo: ~{trajectories_fr1.shape[1] * subsample_factor}")
    print(f"  - Subsampled points per demo: {trajectories_fr1.shape[1]}")
    print(f"  - Speed-up factor: ~{subsample_factor}x")
    print("\nTo adjust speed/accuracy trade-off:")
    print("  - subsample_factor=2: Slower but more accurate")
    print("  - subsample_factor=5: Good balance")
    print("  - subsample_factor=10: Fast (current setting)")
    print("="*70)
...
if __name__ == "__main__":
    ...
    # Subsampling factor for faster execution
    # subsample_factor=2: Use every 2nd point (moderate speed)
    # subsample_factor=5: Use every 5th point (faster)
    # subsample_factor=10: Use every 10th point (very fast, recommended for testing)
    subsample_factor = 2
    ...
    # Run analysis
    model, results, mu_gen, sigma_gen = main(data_path, subsample_factor=subsample_factor)
