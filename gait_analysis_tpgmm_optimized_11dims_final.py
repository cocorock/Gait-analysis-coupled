...
def plot_position_trajectories_xy(trajectories_fr1, mu_generated, sigma_generated):
    ...
    for idx in indices:
        cov_xy = sigma_generated[idx, np.ix_([0, 1], [0, 1])]
    ...
    for idx in indices:
        cov_xy = sigma_generated[idx, np.ix_([5, 6], [5, 6])]
        eigenvalues, eigenvectors = np.linalg.eig(cov_xy)
        angle = np.degrees(np.arctan2(eigenvectors[1, 0], eigenvectors[0, 0]))
        width, height = 2 * 2 * np.sqrt(eigenvalues)
...
def plot_velocity_trajectories_vxvy(trajectories_fr1, mu_generated, sigma_generated):
    ...
    for idx in indices:
        cov_vxvy = sigma_generated[idx, np.ix_([2, 3], [2, 3])]
    ...
    for idx in indices:
        cov_vxvy = sigma_generated[idx, np.ix_([7, 8], [7, 8])]
        eigenvalues, eigenvectors = np.linalg.eig(cov_vxvy)
        angle = np.degrees(np.arctan2(eigenvectors[1, 0], eigenvectors[0, 0]))
        width, height = 2 * 2 * np.sqrt(eigenvalues)
...
def main(data_path, subsample_factor=10, n_jobs=-1):
    ...
    print("  3. ankle_position_trajectories_xy.png - X-Y position plots")
    print("  4. ankle_velocity_trajectories_vxvy.png - Vx-Vy velocity plots")
    print("="*70)
