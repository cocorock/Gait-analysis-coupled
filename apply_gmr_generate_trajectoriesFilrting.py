...
def butter_lowpass_filter(data, cutoff, fs, order=4):
    """
    Apply a low-pass Butterworth filter to trajectory data
...
def generate_trajectory_with_gmr(model, trajectories_fr1, apply_filter=True, 
                                  cutoff_freq=6.0, sampling_freq=200.0, filter_order=4):
    """
    ...
    model : OptimizedTPGMM
        Trained TPGMM model
    trajectories_fr1 : array
        Original demonstration trajectories (for comparison)
    apply_filter : bool
        If True, apply low-pass Butterworth filter to generated trajectory
    cutoff_freq : float
    ...
    time_query : array
        Time vector
    mu_generated : array
        Generated trajectory (filtered if apply_filter=True)
    sigma_generated : array
        Uncertainty covariance matrices
    mu_unfiltered : array
        Unfiltered trajectory (for comparison)
    """
    print("\n" + "="*70)
    ...
    if apply_filter:
        ...
        mu_generated = butter_lowpass_filter(mu_unfiltered, cutoff_freq, 
                                            sampling_freq, filter_order)
        print(f"✓ Filtered trajectory shape: {mu_generated.shape}")
        print(f"✓ Trajectory length preserved: {mu_generated.shape[0] == mu_unfiltered.shape[0]}")
    else:
        mu_generated = mu_unfiltered
        print(f"\n⚠ Filtering disabled - using unfiltered trajectory")
    
    return time_query, mu_generated, sigma_generated, mu_unfiltered
...
def main(model_folder=None, reg_factor=1e-5):
    ...
    # Generate trajectories with GMR (with filtering)
    time_query, mu_generated, sigma_generated, mu_unfiltered = generate_trajectory_with_gmr(
        best_model, trajectories_fr1,
        apply_filter=True,      # Enable filtering
        cutoff_freq=6.0,        # 6 Hz cutoff
        sampling_freq=200.0,    # 200 Hz sampling frequency
        filter_order=4          # Butterworth order 4
    )
    
    # Visualize results (using FILTERED trajectory)
    visualize_results(time_query, mu_generated, sigma_generated, trajectories_fr1)
    
    # Visualize X-Y position trajectories (using FILTERED trajectory)
    plot_position_trajectories_xy(trajectories_fr1, mu_generated, sigma_generated)
    
    # Visualize Vx-Vy velocity trajectories (using FILTERED trajectory)
    ...
    trajectory_file = os.path.join(OUTPUT_FOLDER, 'gmr_trajectory.npy')
    trajectory_unfiltered_file = os.path.join(OUTPUT_FOLDER, 'gmr_trajectory_unfiltered.npy')
    time_file = os.path.join(OUTPUT_FOLDER, 'time_query.npy')
    sigma_file = os.path.join(OUTPUT_FOLDER, 'gmr_sigma.npy')
    
    np.save(trajectory_file, mu_generated)  # Save FILTERED trajectory
    np.save(trajectory_unfiltered_file, mu_unfiltered)  # Save UNFILTERED for comparison
    ...
    print(f"✓ Filtering: Applied (6 Hz cutoff, 200 Hz sampling, order 4)")
    print(f"✓ Total execution time: {overall_time:.2f} seconds")
    print(f"\n✓ All figures saved in folder: {OUTPUT_FOLDER}/")
    print("\n✓ Generated GMR visualizations (FILTERED trajectories):")
    ...
    print(f"  - {trajectory_file} (FILTERED)")
    print(f"  - {trajectory_unfiltered_file} (UNFILTERED)")
    print(f"  - {time_file}")
    print(f"  - {sigma_file}")
    print("="*70)
