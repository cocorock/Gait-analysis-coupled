function [demos, frames] = create_TPGMM_data_FR1(linear_kinematics_pose, frame_index)
    %% create_TPGMM_data_FR1: Transforms kinematics data into a local coordinate frame.
    %
    % Description:
    %   This function transforms each trajectory into a local coordinate system
    %   defined at a specific point (frame_index) along the trajectory.
    %
    % Input:
    %   linear_kinematics_pose - (1 x N_cycles) cell array of original kinematics data.
    %   frame_index            - scalar: The column index (1-200) to use as the origin.
    %
    % Output:
    ...
    for i = 1:num_cycles
        ...
        % Input validation for frame_index
        if frame_index < 1 || frame_index > size(cycle_data.pos, 2)
            error('Invalid frame_index. It must be between 1 and %d.', size(cycle_data.pos, 2));
        end
