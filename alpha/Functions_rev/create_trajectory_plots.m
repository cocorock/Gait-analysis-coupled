%% create_trajectory_plots: Creates and saves plots of original (unfiltered) gait trajectories.
%
% Credits:
%   Victor Ferman, Adrolab FEEC/UNICAMP
%
% Description:
%   This function generates a figure with subplots showing the unfiltered gait
%   trajectories for hip and knee flexion of both legs. It also plots the mean
%   and standard deviation for each joint.
%
% Input:
%   processed_data - struct: Contains the raw gait cycle data.
...
function create_trajectory_plots(processed_data, file_info, save_flag)
    fprintf('\n=== CREATING ORIGINAL TRAJECTORY PLOTS ===\n');
    
    all_right_hip_cycles = processed_data.right_hip_cycles;
    all_left_hip_cycles = processed_data.left_hip_cycles;
    all_right_knee_cycles = processed_data.right_knee_cycles;
    all_left_knee_cycles = processed_data.left_knee_cycles;
    ...
    figure('Name', 'All Gait Cycles from All AMC Files (Unfiltered)', 'Position', [0, 50, 1550, 800]);
...
%     legend(legend_entries, 'Location', 'best', 'FontSize', 8);
    ...
    % Save figure
    if save_flag
        combined_fig_filename = sprintf('Plots and Figs/combined_gait_analysis_unfiltered_%s.png', datestr(now, 'yyyymmdd_HHMMSS'));
        saveas(gcf, combined_fig_filename);
        fprintf('Unfiltered trajectory plot saved as: %s\n', combined_fig_filename);
    end
end
