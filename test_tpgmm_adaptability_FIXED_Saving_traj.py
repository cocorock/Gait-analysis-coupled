...
def visualize_multiple_adaptations(time_baseline, mu_baseline, results_list,
    ...
    for i, result in enumerate(results_list):
        ...
        ax3.plot(time, param, linestyle=linestyle, linewidth=linewidth,
                color=colors[i], label=f'{target/3.333:+.2f}')
...
def visualize_phase_space_plots(time_baseline, mu_baseline, results_list,
    ...
    for i, result in enumerate(results_list):
        ...
        ax.plot(mu_adapted[:, 0], mu_adapted[:, 1], 
               linestyle=linestyle, linewidth=linewidth,
               color=colors[i], alpha=alpha, label=f'{target/3.333:+.2f}', zorder=5)
    ...
    for i, result in enumerate(results_list):
        ...
        ax.plot(time, np.rad2deg(mu_adapted[:, 4]),
               linestyle=linestyle, linewidth=linewidth,
               color=colors[i], alpha=alpha, label=f'{target/3.333:+.2f}')
    ...
    # Get limits from top plots
    pos_xlim = axes[0, 0].get_xlim()
    pos_ylim = axes[0, 0].get_ylim()
    vel_xlim = axes[0, 1].get_xlim()
    vel_ylim = axes[0, 1].get_ylim()
    angle_xlim = axes[0, 2].get_xlim()
    angle_ylim = axes[0, 2].get_ylim()
...
def save_baseline_trajectory_to_csv(time, mu_baseline, output_folder):
    """
    Save the 11-dimensional GMR baseline trajectory to a CSV file
    
    Parameters:
    -----------
    time : array (N,)
        Normalized time vector
    mu_baseline : array (N, 11)
        11-dimensional baseline trajectory from GMR
        Dimensions:
          [0-1]: Right ankle position (x, y)
          [2-3]: Right ankle velocity (x, y)
          [4]:   Right ankle angle (deg)
          [5-6]: Left ankle position (x, y)
          [7-8]: Left ankle velocity (x, y)
          [9]:   Left ankle angle (deg)
          [10]:  Time (normalized)
    output_folder : str
        Path to output folder
    """
    # Define column names for the 11 dimensions
    column_names = [
        'normalized_time',
        'right_ankle_pos_x',
        'right_ankle_pos_y',
    ...
    print(f"  - Dimensions: [0-1] R ankle pos, [2-3] R ankle vel, [4] R ankle angle,")
    print(f"                [5-6] L ankle pos, [7-8] L ankle vel, [9] L ankle angle, [10] time")
...
def main():
    ...
    print(f"\nGenerated files (7 total - 2 per test + baseline CSV):")
    print(f"\n  BASELINE TRAJECTORY:")
    print(f"    0. {output_folder}/baseline_trajectory_11D.csv")
