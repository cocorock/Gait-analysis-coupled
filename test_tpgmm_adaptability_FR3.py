"""
TPGMM ADAPTABILITY TESTING SCRIPT FOR LOWER LIMB EXOSKELETON - FR3 VERSION
===========================================================================

This script tests the adaptability of the trained TPGMM model by applying
time-varying transformations to reference frame FR3 (left foot target).

Tests performed:
1. HORIZONTAL DISPLACEMENT: FR3 shifts from -0.4m to +0.4m along x-axis
2. VERTICAL DISPLACEMENT: FR3 shifts from -0.3m to +0.3m along y-axis  
3. ROTATION: FR3 rotates from -25° to +25°
        ...
        for n in range(N):
            ...
            for k in range(self.K):
                ...
                try:
                    ...
                    # Fallback: use FR3 (the frame with transformations)
                    mean_global, covar_global = self._transform_frame(
                        self.tpgmm.means[2, k],  # FR3 instead of FR2
                        self.tpgmm.covars[2, k],
                        A_n[2],
                        b_n[2]
                    )
...
def create_time_varying_transformations_horizontal(N, displacement_range=(-0.4, 0.4)):
    ...
    FR3 shifts horizontally from displacement_range[0] to displacement_range[1]
    ...
    """
    print("\n" + "="*70)
    print(f"CREATING TIME-VARYING TRANSFORMATIONS: HORIZONTAL DISPLACEMENT (FR3)")
    print("="*70)
    ...
    for n in range(N):
        ...
        # FR2: Right foot target - IDENTITY (no transformation)
        A_frames[n, 1] = np.eye(D)
        b_frames[n, 1] = np.zeros(D)
        
        # FR3: Left foot target - TIME-VARYING HORIZONTAL DISPLACEMENT
        A_frames[n, 2] = np.eye(D)
        b_frames[n, 2, 0] = displacement_x[n]  # Shift in x-direction
        b_frames[n, 2, 1] = 0.0  # No y-shift
    
    print(f"✓ Created time-varying transformations for {N} time steps")
    print(f"  t=0.0: FR3 displacement = {displacement_x[0]:.3f} m")
    print(f"  t=0.5: FR3 displacement = {displacement_x[N//2]:.3f} m")
    print(f"  t=1.0: FR3 displacement = {displacement_x[-1]:.3f} m")
...
def create_time_varying_transformations_vertical(N, displacement_range=(-0.3, 0.3)):
    ...
    """
    print("\n" + "="*70)
    print(f"CREATING TIME-VARYING TRANSFORMATIONS: VERTICAL DISPLACEMENT (FR3)")
    print("="*70)
    print(f"Displacement range: {displacement_range[0]:.2f} to {displacement_range[1]:.2f} m")
    ...
    for n in range(N):
        ...
        # FR3: TIME-VARYING VERTICAL DISPLACEMENT
        A_frames[n, 2] = np.eye(D)
        b_frames[n, 2, 0] = 0.0  # No x-shift
        b_frames[n, 2, 1] = displacement_y[n]  # Shift in y-direction
    
    print(f"✓ Created time-varying transformations for {N} time steps")
    print(f"  t=0.0: FR3 displacement = {displacement_y[0]:.3f} m")
    print(f"  t=0.5: FR3 displacement = {displacement_y[N//2]:.3f} m")
    print(f"  t=1.0: FR3 displacement = {displacement_y[-1]:.3f} m")
...
def create_time_varying_transformations_rotation(N, rotation_range=(-25, 25)):
    ...
    """
    print("\n" + "="*70)
    print(f"CREATING TIME-VARYING TRANSFORMATIONS: ROTATION (FR3)")
    print("="*70)
    ...
    for n in range(N):
        ...
        A_frames[n, 2, 0, 0] = cos_t
        A_frames[n, 2, 0, 1] = -sin_t
        A_frames[n, 2, 1, 0] = sin_t
        A_frames[n, 2, 1, 1] = cos_t
        
        # Rotate right ankle velocity (dims 2, 3)
        A_frames[n, 2, 2, 2] = cos_t
        A_frames[n, 2, 2, 3] = -sin_t
        A_frames[n, 2, 3, 2] = sin_t
        A_frames[n, 2, 3, 3] = cos_t
        
        # Rotate left ankle position (dims 5, 6)
        A_frames[n, 2, 5, 5] = cos_t
        A_frames[n, 2, 5, 6] = -sin_t
        A_frames[n, 2, 6, 5] = sin_t
        A_frames[n, 2, 6, 6] = cos_t
        
        # Rotate left ankle velocity (dims 7, 8)
        A_frames[n, 2, 7, 7] = cos_t
        A_frames[n, 2, 7, 8] = -sin_t
        A_frames[n, 2, 8, 7] = sin_t
        A_frames[n, 2, 8, 8] = cos_t
        
        b_frames[n, 2] = np.zeros(D)
    
    print(f"✓ Created time-varying transformations for {N} time steps")
    print(f"  t=0.0: FR3 rotation = {angle_deg[0]:.1f}°")
    print(f"  t=0.5: FR3 rotation = {angle_deg[N//2]:.1f}°")
    print(f"  t=1.0: FR3 rotation = {angle_deg[-1]:.1f}°")
...
def generate_adapted_trajectory(model, A_frames, b_frames, N=100):
    ...
    print(f"✓ GMR completed successfully!")
    print(f"  Output shape: {mu_out.shape}")
    print(f"  Covariance shape: {sigma_out.shape}")
    
    return time_query.flatten(), mu_out, sigma_out


def visualize_multiple_adaptations(time_baseline, mu_baseline, results, 
                                   test_name, param_name, output_folder):
    """
    Visualize multiple adaptations for a single test type (7 variations)
    """
    print("\n" + "="*70)
    print(f"VISUALIZING RESULTS: {test_name}")
    print("="*70)
    
    # Create figure with 3x3 subplots
    fig = plt.figure(figsize=(20, 16))
    
    # Define colormap for 7 variations
    colors = plt.cm.viridis(np.linspace(0.2, 0.9, 7))
    
    # Plot configurations
    plots = [
        {'idx': 1, 'data_idx': 0, 'title': 'Right Ankle X', 'ylabel': 'X Position (m)'},
        {'idx': 2, 'data_idx': 1, 'title': 'Right Ankle Y', 'ylabel': 'Y Position (m)'},
        {'idx': 3, 'data_idx': 4, 'title': 'Right Ankle Angle', 'ylabel': 'Angle (°)', 'convert_deg': True},
        {'idx': 4, 'data_idx': 5, 'title': 'Left Ankle X', 'ylabel': 'X Position (m)'},
        {'idx': 5, 'data_idx': 6, 'title': 'Left Ankle Y', 'ylabel': 'Y Position (m)'},
        {'idx': 6, 'data_idx': 9, 'title': 'Left Ankle Angle', 'ylabel': 'Angle (°)', 'convert_deg': True},
        {'idx': 7, 'data_idx': None, 'title': param_name, 'ylabel': param_name, 'show_param': True},
        {'idx': 8, 'data_idx': (0, 1), 'title': 'Right Ankle Trajectory', 
         'ylabel': 'Y Position (m)', 'xlabel': 'X Position (m)', 'xy_plot': True},
        {'idx': 9, 'data_idx': (5, 6), 'title': 'Left Ankle Trajectory', 
         'ylabel': 'Y Position (m)', 'xlabel': 'X Position (m)', 'xy_plot': True},
    ]
    
    for plot_cfg in plots:
        ax = plt.subplot(3, 3, plot_cfg['idx'])
        
        if plot_cfg.get('show_param', False):
            # Plot parameter variation
            for i, res in enumerate(results):
                ax.plot(res['time'], res['param'], 
                       color=colors[i], linewidth=2.5, alpha=0.8,
                       label=f"Target: {res['target']:.2f}")
            ax.set_xlabel('Normalized Time', fontweight='bold', fontsize=12)
            ax.legend(fontsize=8, loc='best')
            
        elif plot_cfg.get('xy_plot', False):
            # XY trajectory plot
            x_idx, y_idx = plot_cfg['data_idx']
            
            # Plot baseline
            ax.plot(mu_baseline[:, x_idx], mu_baseline[:, y_idx], 
                   'k--', linewidth=3, label='Baseline', alpha=0.7)
            
            # Plot adaptations
            for i, res in enumerate(results):
                ax.plot(res['mu'][:, x_idx], res['mu'][:, y_idx],
                       color=colors[i], linewidth=2.5, alpha=0.8)
            
            ax.set_xlabel(plot_cfg.get('xlabel', 'X'), fontweight='bold', fontsize=12)
            ax.axis('equal')
            
        else:
            # Time series plot
            data_idx = plot_cfg['data_idx']
            
            # Plot baseline
            baseline_data = mu_baseline[:, data_idx]
            if plot_cfg.get('convert_deg', False):
                baseline_data = np.rad2deg(baseline_data)
            
            ax.plot(time_baseline, baseline_data, 
                   'k--', linewidth=3, label='Baseline', alpha=0.7)
            
            # Plot adaptations
            for i, res in enumerate(results):
                adapted_data = res['mu'][:, data_idx]
                if plot_cfg.get('convert_deg', False):
                    adapted_data = np.rad2deg(adapted_data)
                
                ax.plot(res['time'], adapted_data,
                       color=colors[i], linewidth=2.5, alpha=0.8)
            
            ax.set_xlabel('Normalized Time', fontweight='bold', fontsize=12)
        
        ax.set_ylabel(plot_cfg['ylabel'], fontweight='bold', fontsize=12)
        ax.set_title(plot_cfg['title'], fontweight='bold', fontsize=14)
        ax.grid(True, alpha=0.3)
    
    # Overall title
    fig.suptitle(f'{test_name}: TPGMM Adaptability Test - FR3 Perturbations (7 Variations)', 
                fontsize=20, fontweight='bold', y=0.98)
    
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    
    # Save figure
    save_path = os.path.join(output_folder, f'adaptability_{test_name.lower()}_FR3.png')
...
def visualize_phase_space_plots(time_baseline, mu_baseline, results, test_name, output_folder):
    """
    Create phase space plots showing position vs velocity relationships
    """
    print("\n" + "="*70)
    print(f"VISUALIZING PHASE SPACE: {test_name}")
    print("="*70)
    ...
    # Plot 1: Right ankle X position vs X velocity
    ax = plt.subplot(2, 3, 1)
    ax.plot(mu_baseline[:, 0], mu_baseline[:, 2], 'k--',
           linewidth=2.5, label='Baseline', alpha=0.7)
    for i, res in enumerate(results):
        linewidth = 2.0 if i % 2 == 0 else 1.5
        linestyle = '-' if i < 4 else ':'
        alpha = 0.8 if i % 2 == 0 else 0.6
        ax.plot(res['mu'][:, 0], res['mu'][:, 2],
               linestyle=linestyle, linewidth=linewidth,
               color=colors[i], alpha=alpha)
    ax.set_xlabel('Right Ankle X Position (m)', fontweight='bold', fontsize=12)
    ax.set_ylabel('Right Ankle X Velocity (m/s)', fontweight='bold', fontsize=12)
    ax.set_title('Right Ankle: X Phase Space', fontweight='bold', fontsize=14)
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=9, loc='best')
    
    # Plot 2: Right ankle Y position vs Y velocity
    ax = plt.subplot(2, 3, 2)
    ax.plot(mu_baseline[:, 1], mu_baseline[:, 3], 'k--',
           linewidth=2.5, label='Baseline', alpha=0.7)
    for i, res in enumerate(results):
        linewidth = 2.0 if i % 2 == 0 else 1.5
        linestyle = '-' if i < 4 else ':'
        alpha = 0.8 if i % 2 == 0 else 0.6
        ax.plot(res['mu'][:, 1], res['mu'][:, 3],
               linestyle=linestyle, linewidth=linewidth,
               color=colors[i], alpha=alpha)
    ax.set_xlabel('Right Ankle Y Position (m)', fontweight='bold', fontsize=12)
    ax.set_ylabel('Right Ankle Y Velocity (m/s)', fontweight='bold', fontsize=12)
    ax.set_title('Right Ankle: Y Phase Space', fontweight='bold', fontsize=14)
    ax.grid(True, alpha=0.3)
    
    # Plot 3: Right ankle angle vs time
    ...
    for i, res in enumerate(results):
        ...
        linestyle = '-' if i < 4 else ':'
        alpha = 0.8 if i % 2 == 0 else 0.6
        ax.plot(time, np.rad2deg(res['mu'][:, 4]),
               linestyle=linestyle, linewidth=linewidth,
               color=colors[i], alpha=alpha,
               label=f"Var {i+1}")
    ...
    # Plot 4: Left ankle X position vs X velocity
    ax = plt.subplot(2, 3, 4)
    ax.plot(mu_baseline[:, 5], mu_baseline[:, 7], 'k--',
           linewidth=2.5, label='Baseline', alpha=0.7)
    for i, res in enumerate(results):
        linewidth = 2.0 if i % 2 == 0 else 1.5
        linestyle = '-' if i < 4 else ':'
        alpha = 0.8 if i % 2 == 0 else 0.6
        ax.plot(res['mu'][:, 5], res['mu'][:, 7],
               linestyle=linestyle, linewidth=linewidth,
               color=colors[i], alpha=alpha)
    ax.set_xlabel('Left Ankle X Position (m)', fontweight='bold', fontsize=12)
    ax.set_ylabel('Left Ankle X Velocity (m/s)', fontweight='bold', fontsize=12)
    ax.set_title('Left Ankle: X Phase Space', fontweight='bold', fontsize=14)
    ax.grid(True, alpha=0.3)
    
    # Plot 5: Left ankle Y position vs Y velocity
    ax = plt.subplot(2, 3, 5)
    ax.plot(mu_baseline[:, 6], mu_baseline[:, 8], 'k--',
           linewidth=2.5, label='Baseline', alpha=0.7)
    for i, res in enumerate(results):
        linewidth = 2.0 if i % 2 == 0 else 1.5
        linestyle = '-' if i < 4 else ':'
        alpha = 0.8 if i % 2 == 0 else 0.6
        ax.plot(res['mu'][:, 6], res['mu'][:, 8],
               linestyle=linestyle, linewidth=linewidth,
               color=colors[i], alpha=alpha)
    ax.set_xlabel('Left Ankle Y Position (m)', fontweight='bold', fontsize=12)
    ax.set_ylabel('Left Ankle Y Velocity (m/s)', fontweight='bold', fontsize=12)
    ax.set_title('Left Ankle: Y Phase Space', fontweight='bold', fontsize=14)
    ax.grid(True, alpha=0.3)
    
    # Plot 6: Left ankle angle vs time
    ax = plt.subplot(2, 3, 6)
    for i, res in enumerate(results):
        linewidth = 2.0 if i % 2 == 0 else 1.5
        linestyle = '-' if i < 4 else ':'
        alpha = 0.8 if i % 2 == 0 else 0.6
        ax.plot(time, np.rad2deg(res['mu'][:, 9]),
               linestyle=linestyle, linewidth=linewidth,
               color=colors[i], alpha=alpha)
    ...
    # Overall title
    fig.suptitle(f'Phase Space Analysis: {test_name} - FR3 Perturbations (7 Variations)', 
                fontsize=18, fontweight='bold', y=0.98)
    
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    
    # Save figure with high DPI
    save_path = os.path.join(output_folder, f'adaptability_{test_name.lower()}_phase_space_FR3.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
...
def main():
    ...
    """
    print("\n" + "="*80)
    print(" "*20 + "TPGMM ADAPTABILITY TESTING - FR3 VERSION")
    ...
    output_folder = "adaptability_tests_FR3/"+reg_factor+"/"
    ...
    print("\n\n" + "="*80)
    print(" "*20 + "TEST 1: HORIZONTAL DISPLACEMENT (7 variations) - FR3")
    print("="*80)
    ...
    print("\n\n" + "="*80)
    print(" "*20 + "TEST 2: VERTICAL DISPLACEMENT (7 variations) - FR3")
    print("="*80)
    ...
    print("\n\n" + "="*80)
    print(" "*25 + "TEST 3: ROTATION (7 variations) - FR3")
    print("="*80)
    ...
    print(f"    1. {output_folder}/adaptability_horizontal_displacement_FR3.png")
    print(f"    2. {output_folder}/adaptability_horizontal_displacement_phase_space_FR3.png")
    print(f"\n  VERTICAL DISPLACEMENT:")
    print(f"    3. {output_folder}/adaptability_vertical_displacement_FR3.png")
    print(f"    4. {output_folder}/adaptability_vertical_displacement_phase_space_FR3.png")
    print(f"\n  ROTATION:")
    print(f"    5. {output_folder}/adaptability_rotation_FR3.png")
    print(f"    6. {output_folder}/adaptability_rotation_phase_space_FR3.png")
    print(f"\nTest configurations:")
    ...
    print("\n" + "="*80)
    print("\nKEY FINDINGS:")
    print("- Time-varying transformations successfully applied to FR3")
